...
export async function createGroup(
	...
	const group = await prisma.$transaction(async (tx) => {
		...
		await tx.communityMembership.upsert({
			where: { userId_${groupId}: { userId: ownerId, groupId: newGroup.id } },
			update: { role: 'ADMIN' as any },
			create: { userId: ownerId, groupId: newGroup.id, role: 'ADMIN' as any },
		});
...
export async function getGroupDetails(userId: string, ${groupId}: string) {
	...
	const membership = await prisma.communityMembership.findUnique({
		where: { userId_${groupId}: { userId, ${groupId} } },
	});
...
export async function transferOwnership(
	...
	const membership = await prisma.communityMembership.findUnique({
		where: { userId_${groupId}: { userId: newOwnerId, ${groupId} } },
		select: { id: true },
	});
	if (!membership) {
	...
	return prisma.$transaction(async (tx) => {
		...
		await tx.communityMembership.upsert({
			where: { userId_${groupId}: { userId: newOwnerId, ${groupId} } },
			update: { role: 'ADMIN' as any },
			create: { userId: newOwnerId, ${groupId}, role: 'ADMIN' as any },
		});
...
export async function createGroupShareLink(userId: string, ${groupId}: string): Promise<string> {
	...
	const fallbackUrl = `https://focal.app/community/join/${${groupId}}`;
	...
	try {
		const response = await fetch('https://api2.branch.io/v1/url', {
			...
			body: JSON.stringify({
				...
				feature: 'invite',
				data: {
					'$deeplink_path': `community/join/${${groupId}}`,
					'$og_title': group.name,
