...
const CreateGallerySettingsScreen = () => {
  const navigation = useNavigation<any>();
  const route = useRoute<any>();
  const { name, description, imageUri, groupId } = (route.params || {}) as RouteParams;
  ...
  const onSave = async () => {
    ...
    try {
      const newGallery = await createGallery({
        galleryData: { name, type: 'GROUP', groupId: groupId || undefined },
        imageUri: imageUri ?? null,
      });
      await updateGallery({ galleryId: newGallery.id, data: { ...settingsToSave } as any });
      if (addAllCommunityMembers && groupId) {
        try {
          await addCommunityMembers({ galleryId: newGallery.id, groupId });
        } catch (e) {
          console.warn('Failed to add members automatically', e);
        }
      }
      navigation.navigate('AddGalleryMembers', { galleryId: newGallery.id });
    } catch (e: any) {
      Alert.alert('Failed to create gallery', e?.message ?? 'Please try again.');
    }
  };

  const isLoading = isCreating || isAddingMembers;
  const inherited = inheritCommunitySettings && !!groupId;
...
export default CreateGallerySettingsScreen;