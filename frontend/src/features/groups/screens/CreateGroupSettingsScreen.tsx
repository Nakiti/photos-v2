...
const CreateGroupSettingsScreen = () => {
  ...
  // --- State ---
  const [joinRequiresApproval, setJoinRequiresApproval] = useState<boolean>(true); // Default true for groups
  ...
  const onSave = async () => {
    ...
    try {
      ...
      if (addAllCommunityMembers && communityId) {
        try {
          const result = await addCommunityMembers({ galleryId: newGallery.id, communityId });
          // Optional: handle partial success warnings here
        } catch (memberError) {
          console.warn('Failed to add members automatically', memberError);
        }
      }
